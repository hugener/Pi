// <copyright file="ManagedMemory.cs" company="Pi">
...
namespace Pi.IO.Interop
{
    ...
    public class ManagedMemory : IMemory
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="ManagedMemory"/> class.
        /// </summary>
        /// <param name="length">The length.</param>
        public ManagedMemory(int length)
        {
            this.byteArray = new byte[length];
            this.handle = GCHandle.Alloc(this.byteArray, GCHandleType.Pinned);
            this.memoryPointer = this.handle.AddrOfPinnedObject();
        }
        ...
        {
            get => this.byteArray[index];
            set => this.byteArray[index] = value;
        }
        ...
        {
            Array.Copy(source, sourceIndex, this.byteArray, destinationIndex, length);
        }
        ...
        {
            Array.Copy(this.byteArray, sourceIndex, destination, destinationIndex, length);
        }
        ...
        protected virtual void Dispose(bool disposing)
        {
            ...
            Trace.Assert(
                disposing,
                string.Format("ERROR: GC finalized managed memory of {0} bytes for address {1} that was not disposed!", this.byteArray.Length, this.memoryPointer.ToString("X8")));
